    ...
    public function connectToDatabases ( $username='Guest', $dbsConfigFile=null ) {
        ...
        if (true || ($username=='admin' || (is_object($this->findConnection('couchdb')) && is_null($this->findConnection('couchdb')->username)))) {
            ...
            if (!file_exists($domainConfigsPath.$configFilename)) {
                $configFilename2 = 'databases.username-Guest.json';
                trigger_error ($fncn.' : !file_exists("'.$domainConfigsPath.$configFilename.'"), using "'.$domainConfigsPath.$configFilename2.'" instead.', E_USER_WARNING);
