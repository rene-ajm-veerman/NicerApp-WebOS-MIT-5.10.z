<?php
$fn = realpath(dirname(__FILE__).'/../../../../../..').'/NicerAppWebOS/functions.php';
//var_dump ($fn); die();
require_once ($fn);
...
if (is_array($view) && array_key_exists($appFolder, $view)) {
    ...
    if ($view[$appFolder]['set']=='index') {
        require (dirname(__FILE__).'/frontpage.php');
    } else {
        echo '<pre style="color:purple">'; debug_print_backtrace(); echo '</pre>';
        require (dirname(__FILE__).'/app.dialog.siteContent_seeAndPlayMusicFolder.php');
    }
} else {
        echo '<pre>addsc1::'; var_dump($view); var_dump ($appFolder); echo '</pre>';
    /*if ($view['set']=='index')
