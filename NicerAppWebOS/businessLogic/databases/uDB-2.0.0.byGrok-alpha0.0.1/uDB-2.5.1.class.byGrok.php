...
class uDB2
{
    ...
    /**
     * Robust conversion - fixes most "stdClass as array" errors
     */
    ...
    /**
     * Safe property/key check (works for both objects and arrays)
     */
    public function hasProperty($data, $key) {
        if (is_object($data)) {
    ...
    public function __get($name) {
        if (in_array($name, ['cdb', 'db', 'database'])) {
            if ($this->isCouch() && $this->couchConnector !== null) {
                return $this->couchConnector;
            }
        }
        return null;
    }
    ...
    public function setDatabase($databaseName) {
        if ($this->isCouch() && $this->couchConnector) {
            if (method_exists($this->couchConnector, 'setDatabase')) {
                return $this->couchConnector->setDatabase($databaseName);
            }
        }

        // Debug output during development
        if (isset($GLOBALS['naWebOS']) && $GLOBALS['naWebOS']->showAllErrors) {
            trigger_error("uDB2->setDatabase('$databaseName') called but couchConnector is null", E_USER_WARNING);
        }
        return false;
    }

    public function getCouchDB() {
        return $this->__get('cdb');
    }
