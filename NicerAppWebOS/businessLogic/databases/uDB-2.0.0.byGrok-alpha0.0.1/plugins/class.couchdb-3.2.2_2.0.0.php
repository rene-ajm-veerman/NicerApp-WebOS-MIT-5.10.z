...
//echo '<pre style="background:blue;color:white;">'; var_dump ($_COOKIE);echo '</pre>';
    ...
    // These methods were commonly called directly on the old database connector

    public function setDatabase($dbName = "") {
        if (empty($dbName)) {
            $dbName = $this->database ?? $this->currentDatabase ?? '';
        }
        if (!empty($dbName) && $this->cdb) {
    ...
    public function find($relTableName = null, $findCommand = null, $options = []) {
        if (!$this->cdb) {
            trigger_error("find() called but cdb is null", E_USER_WARNING);
            return ['result' => false, 'docs' => []];
        }

        if (empty($relTableName)) $relTableName = $this->getDatabase();

        try {
            if (method_exists($this->cdb, 'find')) {
                return $this->cdb->find($relTableName, $findCommand, $options);
            } elseif (method_exists($this->cdb, 'getView')) {
                return $this->cdb->getView($relTableName, '_all_docs', $findCommand);
            }
            return ['result' => false, 'docs' => []];
        } catch (Exception $e) {
            trigger_error("CouchDB find() error: " . $e->getMessage(), E_USER_WARNING);
            return ['result' => false, 'error' => $e->getMessage(), 'docs' => []];
        }
    }

    public function findOne($relTableName = null, $findCommand = null) {
        $result = $this->find($relTableName, $findCommand);
        return $result['docs'][0] ?? null;
    }

    public function get($id, $options = []) {
        if (!$this->cdb) {
            trigger_error("get() called but cdb is null", E_USER_WARNING);
            return null;
        }
        try {
            if (method_exists($this->cdb, 'get')) {
                return $this->cdb->get($id, $options);
            } elseif (method_exists($this->cdb, 'getDocument')) {
                return $this->cdb->getDocument($id, $options);
            }
            return null;
        } catch (Exception $e) {
            trigger_error("CouchDB get() error for '$id': " . $e->getMessage(), E_USER_WARNING);
            return null;
        }
    }

    public function getAll($relTableName = null, $options = []) {
        return $this->find($relTableName, ['selector' => (object)[]], $options);
    }

    public function insert($relTableName, $document) {
        if (!$this->cdb) return false;
        try {
            return $this->cdb->insert($relTableName, $document);
        } catch (Exception $e) {
            return false;
        }
    }

    public function update($relTableName, $document) {
        if (!$this->cdb) return false;
        try {
            return $this->cdb->update($relTableName, $document);
        } catch (Exception $e) {
            return false;
        }
    }
    ...
    public function dataSetName($relTableName) {
        return $relTableName;   // Many old calls expect this
    }

    // Catch any other unknown method calls gracefully
    public function __call($method, $args) {
        trigger_error("Unknown legacy method called: $method() on CouchDB connector", E_USER_WARNING);
        return null;
    }}
?>
